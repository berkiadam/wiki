...
```
5. docker run -d --name elasticsearch \
-p 9200:9200 -p 9300:9300 -e "discovery.type=single-node" \
docker.elastic.co/elasticsearch/elasticsearch:6.4.0

6. docker cp -L elasticsearch:/usr/share/elasticsearch/config/ /home/adam/Projects/DockerCourse/persistentstore/elasticsearch/
7. chmod 777 /home/adam/Projects/DockerCourse/persistentstore/elasticsearch/config
8. mkdir /home/adam/Projects/DockerCourse/persistentstore/elasticsearch/data

9. docker rm -f elasticsearch
```
...
```
```
10. docker service ps elasticsearch 
ID                  NAME                IMAGE                                                 NODE 
...
```
13. eval $(docker-machine env mg0)
14. docker service rm logger-test 
```
...
```
16. docker service logs logspout
...
```
17. docker service logs -f logstash
```
...
**Cluster információ:**
```
19. curl -XGET 192.168.123.71:9200/_cat/health?v
...
**Node-ok listája:**
```
20. curl -XGET http://192.168.123.71:9200/_cat/nodes?v
...
```
21. curl -XGET http://192.168.123.71:9200/_cat/indices?v
health status index               uuid                   pri rep docs.count docs.deleted store.size pri.store.size
...
```
23. curl -XGET http://192.168.123.71:9200/_cat/indices?v
health status index               uuid                   pri rep docs.count docs.deleted store.size pri.store.size
...
...
### Hisztogram
Ez egy nem kommulált hisztogramot képez automatikusan a megadott indxeben lévő dokumentumok egy mezőjéből. A végeredmény egy több vödrös aggregáció. A vödrök számát, vagyis a hisztogram csoportokat magától számolja ki, nekünk csak a felbontást (lépés méretet) kell megadni. A hisztogramokról bővebben itt olvashatunk: [\2](\1)
